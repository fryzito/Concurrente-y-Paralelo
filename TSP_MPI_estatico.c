...
int main(int argc, char* argv[]) {
  ...
  Read_digraph(digraph_file);   // Lee el grafo desde archivos
  
  if (my_rank == 0) {
    // Escribimos la matris leida
    int p,q;
    printf("Numero de nodos del Grafo: %d\n",n);
    printf("Matris=\n");
    for(p=0;p<n;p++)
      for(q=0;q<n;q++){
	printf("%d%c",digraph[n*p+q],(char)q==n-1?10:32);
      }
  ...
  MPI_Pack_size(1, MPI_INT, comm, &one_msg_sz);
     mpi_buffer =  malloc(100*comm_sz*(one_msg_sz + MPI_BSEND_OVERHEAD)*sizeof(char));
     MPI_Buffer_attach(mpi_buffer, 100*comm_sz*(one_msg_sz + MPI_BSEND_OVERHEAD));

     start = MPI_Wtime();

     Par_tree_search();    // esta parte produce error
...
void Par_tree_search(void) {
  ...
  Partition_tree(stack);  // esta parte falta ver que esta mal
...
void Partition_tree(my_stack_t stack) {
  ...
  Set_init_tours(init_tour_count, counts, displacements, &my_count, &tour_list);
...
void Build_initial_queue(city_t** queue_list_p, int queue_size, int* init_tour_count_p) {
  my_queue_t queue;
  int curr_sz = 0, i;
  city_t nbr;
  tour_t tour = Alloc_tour(NULL);
...
void Look_for_best_tours(void) {
  ...
  while(!done) {
    MPI_Iprobe(MPI_ANY_SOURCE, TOUR_TAG, comm, &msg_avail, &status);
    if (msg_avail) {
      MPI_Recv(&tour_cost, 1, MPI_INT, status.MPI_SOURCE, TOUR_TAG,comm, MPI_STATUS_IGNORE);
...
void Cleanup_msg_queue(void) {
  ...
  sprintf(string1, "Messages not received:  unknown = %d, tour = %d", counts[0], counts[1]);
  // printf("Proc %d > %s\n", my_rank, string1);
}
